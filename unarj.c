 ...
 * 04/29/91  R. Jung        Added l command.  Removed 16 bit dependency
 *                          in fillbuf().
 ...
 * 09/02/93  K. Flint       Added wildcards and extraction paths.
 * 01/24/94  K. Flint       Added wildcard expansion to archive file path
 * 01/31/98  T. Okabayashi  Added p command (extract to stdout).
 *
 */
...
static char *M_USAGE  [] =
{
"Usage: UNARJ <archive>[.arj]    (list archive) or \n",
"       UNARJ <Command> <archive>[.arj] [outpath][fs1] [path][fs2] ...\n",
"       Commands:   E    (extract archive)     L    (list archive)\n",
"                   T    (test archive)        X    (extract with pathnames)\n",
"                   P    (extract to stdout)                               \n",
"       outpath: Changes extract path for all files (default = current dir)\n",
"       path: overides outpath for next filespec only\n",
"       fs: filespec - file[s] to extract (wildcards *,? OK)\n",
"\n",
...
char M_VERSION [] = "UNARJ (Modified Demo version) 2.41.to.b Copyright (c) 1991-93 Robert K Jung\nModifed by Kerry Flint (2.41.kkf.a) / T. Okabayashi(2.41.to.b) \n\n";
...
static char   extract_to_dir[FNAME_MAX];
static char   comment[COMMENT_MAX];
static char   *hdr_filename;
static char   *hdr_comment;
static char   **extract_files;
static int    num_extract_files = 0;
...
static int check_wild(char *wild, char *check){
/* compare a wildcard string with another string */

   char *tmp;

   tmp = strrchr(wild, '\\'); /* split off path */
   if (tmp) 
      wild = tmp + 1;

   if (command == 'D')   printf("Comparing %s and %s : ", wild, check);
   while ( ((myupper(*wild) == myupper(*check) ) || *wild == '*' || *wild == '?') && *wild) {
      if (myupper(*wild) == myupper(*check)) {
         wild++;
         check++;
      } else { /* wild card */
         if (*wild == '?') { /* match any single char */
            if (*check != '.' && *check ){ /* don't increment if next char . or \0 */
               check++;
            }
            wild++;
         } else { /* * match any number of characters */
            while (*wild == '*' || *wild == '?') { /* skip to next non wild */
               wild++;
            } /* endwhile */
            if (!*wild) { /* matches to EOS */
               while (*check) {
                  check++;
               } /* endwhile */
            } else {
               tmp = check;
               while (*tmp) { /* find last char that matches */
                  if (myupper(*tmp) == myupper(*wild)){
                     check = tmp;
                  }
                  tmp++;
               }
               if (*wild == '.' && *check != '.'){ /* make EOS dot pos */
                  check = tmp;
                  wild++;
                }
            } /* endif */
   ...
   if (*wild == *check) {
       if (command == 'D')      printf("Match\n");
      return(1);
   } else {
      if (command == 'D')      printf("No-Wild=%s & Check =%s\n", wild, check);
      return(0);
   } /* endif */
}

static int file_check(char *name){

   int i, len, n_len, e_len;
   char *ext;

   if (!num_extract_files) {
      return(1);
   } /* endif */
   for (i=0;i < num_extract_files; i++ ) {
#ifdef SIMPLE_WILDS
      /* exact match */
      if (stricmp(name, extract_files[i]) == 0)
         return(1);
      /* Every file wild card */
      if (strcmp("*.*", extract_files[i]) == 0 ||
          strcmp("*", extract_files[i]) == 0) {
         return(1);
      } /* endif */
      /* check just extensions */
      if (strncmp("*.", extract_files[i], 2) == 0) {
         ext = strrchr(name, '.');
         if (ext){
            if (stricmp(ext, &extract_files[i][2]) == 0){
               return (1);
            }
         }
      } else {
         /* any extension */
         len = strlen(extract_files[i]);
         if (strcmp(".*", &extract_files[i][len -2]) == 0){
            ext = strrchr(name, '.');
            if (ext) {
               e_len = strlen(ext);
            } else {
            ...
            n_len = strlen(name) - e_len;
            if (strnicmp(name, extract_files[i], len-2) == 0 && (n_len == len - 2)){
               return(1);
            }
         }
      } /* endif */
#else
      if (check_wild(extract_files[i], name)){
         strcpy(extract_to_dir, extract_files[i]); /* figure out which extract path to use */
         ext = strrchr(extract_to_dir, '\\');
         if (ext) {
            *(ext+1) = '\0';
         } else {
            strcpy(extract_to_dir, extract_files[0]);
            ext = strrchr(extract_to_dir, '\\');
            if (ext) {
               *(ext+1) = '\0';
            } else {
               strcpy(extract_to_dir, DEFAULT_DIR);
            }
         } /* endif */
         return(1);
      }
...
execute_cmd()
{
   ...
   ULONG FindCount, File_Attribute;
   APIRET rc;
   char FileSpec[FNAME_MAX];
   char PathSpec[FNAME_MAX];

   FindHandle = 0x0001;
   FindCount = 1;
   File_Attribute = NORMAL_FILES | FILE_ARCHIVED | FILE_HIDDEN | FILE_READONLY;  /* All files, no dirs */

   strncopy(FileSpec, arc_name, FNAME_MAX);
   parse_path(FileSpec, PathSpec, NULL);
   rc = DosFindFirst(FileSpec, &FindHandle, File_Attribute, (PVOID) &FindBuffer,
                     sizeof(FindBuffer), &FindCount, FIL_STANDARD);
   if (rc != 0)
        error(M_CANTOPEN, arc_name);
   while (rc == 0) {
      strncopy(arc_name, PathSpec, FNAME_MAX);
      strncat(arc_name, FindBuffer.achName, FNAME_MAX - strlen(PathSpec) - 1);
      arc_name[FNAME_MAX -1] = '\0';
   #endif 
      first_hdr_pos = 0;
      time_stamp = 0;
      ...
      if(command != 'P') printf(M_PROCARC, arc_name);
      ...
      {
         strcpy(extract_to_dir, DEFAULT_DIR);
         doit = file_check(filename+entry_pos);
         if (!doit)
            skip();
         else switch (command)
         {
...
char *argv[];
{
    ...
    else if (argc >= 3)
    {
        ...
        if (strchr("ELTXDP", command) == NULL) 
        {
            printf(M_VERSION); 
            error(M_BADCOMND, argv[1]);
        }
        if(command != 'P') printf(M_VERSION);
        arc_p = argv[2];
        extract_files = &argv[3];
        num_extract_files = argc - 3;
    }
    else
    {
        printf(M_VERSION); 
